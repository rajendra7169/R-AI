...
if [ -f "$OLLAMA_BIN" ] && is_native_binary "$OLLAMA_BIN" && file_ok "$OLLAMA_BIN" 10000000; then
    ...
    if copy_from_drive_root "ollama-darwin" "$OLLAMA_BIN" 10000000; then
        if is_native_binary "$OLLAMA_BIN"; then
            ...
            echo -e "${RED}      WARNING: Copied file is not a valid Mach-O binary.${RST}"
            rm -f "$OLLAMA_BIN"
        fi
    fi
    if [ ! -f "$OLLAMA_BIN" ]; then
        echo -e "      Downloading Ollama engine (streaming - stops after binary extracted)..."
        # 'ollama' is the first entry in the tgz, so tar exits as soon as it's
        # extracted and the rest of the archive download is skipped.
        curl -L --fail "$ARCHIVE_URL" | tar -xzf - -C "$SHARED_BIN" ollama 2>/dev/null
...
# ── Final ──────────────────────────────────────────────────────
START_HINT="To start your AI: Double-click  Mac/start.command"
