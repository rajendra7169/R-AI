...
async function saveLogMode() {
  ...
  toast(S.logMode === "all" ? "Logging: everything" : "Logging: errors only");
}
...
async function updateEngineStatus() {
  try {
    ...
    if (!sdReady) {
      ...
      D.imgWarn.innerHTML =
        "<strong>⚠️ Missing VC++ Redistributable</strong><br>The image engine needs Microsoft Visual C++ Redistributable.<br>Re-run the Windows installer or download it from Microsoft.";
    ...
    D.imgWarn.innerHTML =
      "<strong>⚠️ Offline</strong><br>Cannot reach the server.";
    D.imgWarn.style.display = "block";
    D.imgGenBtn.disabled = true;
  }
}
...
function showImgResult(data) {
  ...
  D.imgParamsGrid.innerHTML = rows
    .map(
      (r) =>
        `<div class="img-param"><span class="img-param-key">${esc(r.k)}</span><span class="img-param-val">${esc(String(r.v))}</span></div>`,
    )
    .join("");
}
...
async function generateImage() {
  ...
  try {
    ...
    if (!startR.ok || startD.error) {
      ...
      hideImgProgress();
      D.imgResult.style.display = "none";
      D.imgPreview.insertAdjacentHTML(
        "beforeend",
        `<div class="img-status" style="color:var(--red); margin-top:12px;">${esc(msg)}</div>`,
      );
      D.imgGenBtn.disabled = false;
    ...
    pollInterval = setInterval(async () => {
      ...
      try {
        const pr = await fetch(
          `/api/image-progress?job_id=${encodeURIComponent(jobId)}`,
        );
        ...
        if (pd.status === "done") {
          ...
          D.imgPreview.insertAdjacentHTML(
            "beforeend",
            `<div class="img-status" style="color:var(--red); margin-top:12px;">${esc(pd.error || "Generation failed.")}</div>`,
          );
          D.imgGenBtn.disabled = false;
        } else {
    ...
    D.imgPreview.insertAdjacentHTML(
      "beforeend",
      `<div class="img-status" style="color:var(--red); margin-top:12px;">Network error: ${esc(e.message || "Failed to generate image.")}</div>`,
    );
    D.imgGenBtn.disabled = false;
...
async function stopChatEngine() {
  ...
  try {
    ...
    toast(
      stopped
        ? "Chat engine unloaded."
        : "Chat engine stop timed out — may still be running.",
    );
  } catch {
...
function renderAttachmentBar() {
  ...
  // inline !important.
  const previewStyle =
    "width:60px;height:60px;max-width:60px;max-height:60px;overflow:hidden;position:relative;border-radius:10px;border:1px solid var(--bd);display:inline-block;flex:0 0 60px;";
  const imgStyle =
    "width:60px;height:60px;max-width:60px;max-height:60px;object-fit:cover;display:block;";
...
async function sendMsg(text) {
  ...
  const imageAttachments = S.attachments.filter((a) => a.type === "image");
  ...
  if (conv.msgs.length === 1)
    conv.title =
      text.trim().substring(0, 40) + (text.trim().length > 40 ? "..." : "");
...
async function streamOllama(conv) {
  ...
  try {
    ...
    if (contentEl) {
      const renderedContent = aiMsg.content ? renderMd(aiMsg.content) : "";
      const renderedThinking = aiMsg.thinking
        ...
        : "";
      contentEl.innerHTML =
        renderedThinking + renderedContent ||
...
function bind() {
  ...
  if (hwWrap && hwToggle) {
    const setOpen = (open) => {
      ...
      try {
        localStorage.setItem("g-hw-open", open ? "1" : "0");
      } catch (_) {}
    };
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (D.imgModal.classList.contains("on")) {
      closeImgModal();
      return;
    }
    if (D.sysOverlay.classList.contains("open")) {
      closeSysPanel();
      return;
    }
    if (S.editing) {
